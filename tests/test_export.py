...
class InputTest(unittest.TestCase):
    ...
    def test_other_courses_do_not_reuse_cached_content(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            args.output = None
            with (
                patch("netagrab.cli.Path", side_effect=lambda value: out if value == "output" else Path(value)),
                redirect_stdout(io.StringIO()),
            ):
...
class ExportTest(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as temp, patch("netagrab.exporter.SESSION", Path(temp) / "auth.json"):
            ...
            def respond(route):
                if "auth.netacad.com" in route.request.url:
                    ...
                    route.fulfill(
                        content_type="text/html",
                        body='<script>location.replace("https://auth.netacad.com/login")</script>',
                    )
            ...
            with (
                ...
                patch("builtins.input", return_value="student@example.invalid") as email,
    ...
    def test_nested_tabs_tables_graphics_pdf_and_offline_links(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            manifest = {
                "content_base": "https://example.invalid/courses/content/",
                "language": "en-US",
                "modules": [{"number": 1}],
            }
            ...
            graphic = {
                "_id": "nested",
                "_component": "graphic",
                "_graphic": {"src": "en-US/assets/sample.png"},
                "body": "Nested diagram explanation",
            }
            ...
            table = {
                "_id": "table",
                "_component": "table",
                "_rows": [{"_cells": [{"text": "Destination", "_isHeading": True}, {"text": "192.0.2.1"}]}],
            }
            ...
            merge_pdfs(
                [
                    {"pdf": str(root / "test.pdf"), "title": "Module one"},
                    {"pdf": str(root / "test.pdf"), "title": "Lab handout"},
                ],
                root / "merged.pdf",
            )
    ...
    def test_markdown_preserves_tables_code_images_and_external_links(self):
        md = to_markdown(
            "<style>unwanted css</style><h2>Routing</h2>"
            "<pre>Router&gt; enable\n  show ip route</pre>"
        ...
        for formats in [("markdown",), ("json",), ("pdf", "markdown", "json")]:
            with self.subTest(formats=formats), tempfile.TemporaryDirectory() as temp:
                ...
                data = {
                    ...
                    "assets": [],
                    "contentObjects": [
                        {"_id": "section-1", "_parentId": "course", "_type": "page", "title": "1.1 Routing"}
                    ],
